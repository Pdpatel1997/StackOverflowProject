...
namespace StackOverflowProject.Repositories
{
    ...
    class AnswersRepository : IAnswersRepository
    {
        ...
        {
            Answer ans = db.Answers.Where(s => s.AnswerID == a.AnswerID).FirstOrDefault();
            if (ans != null)
            {
        ...
        {
            Answer ans = db.Answers.Where(s => s.AnswerID == aid).FirstOrDefault();
            if (ans != null)
            {
        ...
        {
            List<Answer> ans = db.Answers.Where(s => s.QuestionID == qid).OrderByDescending(s => s.AnswerDateAndTime).ToList();
            return ans;
        }
        public List<Answer> GetAnswersByAnswerID(int aid)
        {
            List<Answer> ans = db.Answers.Where(s => s.AnswerID == aid).OrderByDescending(s => s.AnswerDateAndTime).ToList();
            return ans;
        }
    }
}
